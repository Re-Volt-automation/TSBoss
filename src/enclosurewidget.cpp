...
static double           g_C = TSCalculator::C;  // speed of sound [m/s], adjustable
...
static bool hasPortedData(const BoxModel &m)
{
...
static Cpx portedUa(const BoxModel &m, double f)
{
...
static double portedSplRaw     (const BoxModel &m, double f)
{ return (2.0*PI*f)*std::abs(portedUa(m,f)); }

static double portedConeSplRaw (const BoxModel &m, double f)
{ auto a=portedAmplitudes(m,f); return (2.0*PI*f)*std::abs(a.cone); }

static double portedPortSplRaw (const BoxModel &m, double f)
{ auto a=portedAmplitudes(m,f); return (2.0*PI*f)*std::abs(a.port); }

// Group delay [ms] via numerical phase derivative of Ua
static double portedGroupDelay(const BoxModel &m, double f)
{
...
static double portArea_m2(const BoxModel &m)
{
    if (m.portShape == 0) {
...
static double portAirVelocity(const BoxModel &m, double f, double power)
{
...
static bool hasPortVelocityData(const BoxModel &m)
{
    return isVented(m) && hasPortedData(m) && m.Re > 0 && portArea_m2(m) > 0;
}
